...
### Requirement: Cold-import protection at test boundary

The test suite SHALL include a warm-up import in `tests/conftest.py` that
initializes `commitizen.changelog_formats` before any test module imports the
plugin, eliminating the cold-import path during test collection.

#### Scenario: Test collection succeeds

- **WHEN** pytest collects `tests/test_spdx_markdown.py`
- **THEN** the import of `SPDXMarkdown` SHALL succeed without `AttributeError`

## REMOVED Requirements

### Requirement: Staggered import order with E402 suppressions

**Reason**: The ABC wrapper pattern that required `SPDXMarkdown` to be defined
before any commitizen imports is no longer used. The direct subclass with
conftest warm-up handles the cold-import condition at the test boundary instead
of in production code.

**Migration**: Removed `import abc`, `_resolve()` classmethod, `__new__`
override, and `SPDXMarkdown.register(_SPDXMarkdownImpl)` call. Normal top-level
imports restored.
